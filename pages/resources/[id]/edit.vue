...
const fetchResource = async () => {
  try {
    ...
    if (data.success) {
      ...
      serverError.value = data.message || 'Nu s-a putut încărca resursa'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      serverError.value = data.message || 'Nu s-a putut actualiza resursa'
      toast.error(serverError.value)
    }
  } catch (error) {
...
<template>
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-900">Editează Resursa</h1>
      <NuxtLink 
        to="/resources" 
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div>
          <div class="flex items-center">
            ...
            />
            <label for="isAvailable" class="ml-2 block text-sm text-gray-700">Resursa este disponibilă pentru rezervare</label>
          </div>
        </div>
        ...
        <div class="flex items-center justify-end pt-4 space-x-4">
          <NuxtLink
            to="/resources"
            class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 font-medium hover:bg-gray-50 transition"
          >
            Anulează
          </NuxtLink>
          <button
            type="submit"
            class="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-md font-medium transition flex items-center"
            :disabled="isLoading"
          >
            ...
            {{ isLoading ? 'Se actualizează...' : 'Actualizează Resursa' }}
          </button>
        </div>
      </form>
    </div>
