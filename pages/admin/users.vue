...
const { user } = useAuth()
const { users, isLoading, error, fetchUsers, updateUser, deleteUser, clearError } = useUsers()
...
const handleConfirmDelete = async () => {
  ...
  try {
    ...
    successMessage.value = 'Utilizatorul a fost șters cu succes!'
...
<template>
  <div class="container mx-auto px-4 py-8">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <div class="flex space-x-3">
                  ...
                  <button
                    ...
                    @keydown="(event) => handleKeyDown(event, handleDeleteUser, userItem)"
                    class="text-red-600 hover:text-red-900 transition-colors duration-200"
                    tabindex="0"
                    :aria-label="`Delete user ${userItem.fullName}`"
                    :disabled="userItem._id === user._id"
                    :class="{ 'opacity-50 cursor-not-allowed': userItem._id === user._id }"
                  >
                    Șterge
                  </button>
                </div>
    ...
    <div v-if="isDeleteModalOpen" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
          <!-- Modal Header -->
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-medium text-gray-900">Confirmă Ștergerea</h3>
            <button
              @click="handleCloseDeleteModal"
              @keydown="(event) => handleKeyDown(event, handleCloseDeleteModal)"
              class="text-gray-400 hover:text-gray-600 transition-colors duration-200"
              tabindex="0"
              aria-label="Close modal"
            >
          ...
          <div v-if="deletingUser && !successMessage" class="mb-6">
            <div class="flex items-center mb-4">
              ...
              <div class="ml-4">
                <h4 class="text-lg font-medium text-gray-900">Ești sigur că vrei să ștergi acest utilizator?</h4>
                <p class="text-sm text-gray-600 mt-1">
                  Această acțiune nu poate fi anulată. Utilizatorul 
                  <span class="font-semibold">{{ deletingUser.fullName }}</span> 
                  ({{ deletingUser.email }}) va fi șters permanent din sistem.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-end space-x-3 pt-4">
            <button
              ...
              @click="handleCloseDeleteModal"
              @keydown="(event) => handleKeyDown(event, handleCloseDeleteModal)"
              class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-200"
              tabindex="0"
              :disabled="isDeleting"
            >
              Anulează
            ...
            <button
              ...
              @keydown="(event) => handleKeyDown(event, handleConfirmDelete)"
              class="px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
              tabindex="0"
              :disabled="isDeleting"
            >
              ...
              <span v-else>Șterge Utilizator</span>
            </button>
          </div>
        </div>
      </div>
